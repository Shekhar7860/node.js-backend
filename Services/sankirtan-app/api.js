      ...
      .then(async (result) => {
        if (result) {
          let selected =  await Setting.findOne({
            where: { key: 'security.super_admin' }
           })
           ...
           Setting.update(setting, { where: { key: 'security.super_admin' } })
           .then((result) => {
             if (result) {
...
router.get("/goal-setting", async function (req, res) {
  ...
  // }
    ...
    data = {
      ...
      created_by_name: summaryDetails.createdBy.firstname + ' '+ summaryDetails.modifiedBy.lastname,
      last_modified_by_name: summaryDetails.modifiedBy.firstname + ' '+ summaryDetails.modifiedBy.lastname,
      goal_description
    };
    res.status(200).send(data);
  } else {
  //   BusinessPlanSummary.findOne
  //   ({ 
  //    where: { group_id: group_id, year: year },
  //   include: [{
  //      model: People,
  //      as: 'createdBy',
  //      attributes: ["preferred_name","firstname", "middle_name","lastname"],
        ...
        if (
          ...
          itemData.other_distributor_ids.map((other_distributor_id) => {
            addUpdateTransactionPeople(
              ...
              { transaction_id, other_distributor_id }
            );
          });
        }
       // sending mail
       if(itemData.distribution_list_email !== undefined && itemData.email_book_distribution_entry !== undefined) {
         if(itemData.email_book_distribution_entry == true && itemData.distribution_list_email !== null)
         {
        sendEmail(itemData, itemData.distribution_list_email)
         }
       }
        res.status(200).send(result);
      } else {
...
// });
   ...
   // console.log('insidedata', transactionData);
     ...
     //  console.log('tt', t)
      ...
      //       let result1 = []
      //       itemData.selected.map((sel, key) => {
      //         JSON.parse(result1.dataValues.photos2[key]).selected = sel
...
sendEmail = (data, email) => {
  ...
  var mailOptions = {
    ...
    <div style=" margin:0 0 15px 0;font-weight:normal">
        ...
        <span style="padding:0 0 0 10px;">${data.location_address, data.location_city, data.location_state}</span>
      </div>
      <div style=" margin:0 0 15px 0;font-weight:normal">
        <span style="font-weight:bold;min-width: 150px; display: inline-block;">Date </span>: 
        <span style="padding:0 0 0 10px;">${moment(data.date).format('MM-DD-YYYY')}</span>
      </div>
      <div style="margin:0 0 15px 0;font-weight:normal">
...
router.get("/import-book-distribution", function (req, res) {
  const { email_book_distribution_entry, distribution_list_email } = req.query;
                              ...
                              this.addUpdateTransactionLineItem(
                                {
                                  ...
                                  transaction_book_points: row["transaction_book_points"],
                                  organization_id,
                                },
...
router.patch("/update-settings-data", async function (req, res){
  ...
  if(req.body.key == "security.super_admin")
  {
    const settings = await Setting.findOne({ where: { key: 'security.super_admin' } });
   // console.log('settings are', settings.dataValues.value);
